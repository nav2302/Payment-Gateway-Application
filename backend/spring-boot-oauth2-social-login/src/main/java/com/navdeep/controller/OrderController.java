...
import com.navdeep.config.RazorPayClientConfig;
import com.navdeep.dto.ApiResponse;
import com.navdeep.dto.LocalUser;
import com.navdeep.dto.payment.OrderRequest;
import com.navdeep.dto.payment.OrderResponse;
import com.navdeep.dto.payment.PaymentResponse;
import com.navdeep.service.OrderService;
...
public class OrderController {
	...
	public ResponseEntity<?> createOrder(@RequestBody OrderRequest orderRequest, @CurrentUser LocalUser user) {
		...
		try {
			// The transaction amount is expressed in the currency Paise (in case of INR)
	...
	public ResponseEntity<?> updateOrder(@RequestBody PaymentResponse paymentResponse) {
		String errorMsgFromMatchingSignature = orderService.validateAndUpdateOrder(paymentResponse.getRazorpayOrderId(), 
				paymentResponse.getRazorpayPaymentId(), paymentResponse.getRazorpaySignature(),
				razorPayClientConfig.getSecret());
		if (errorMsgFromMatchingSignature != null) {
			return new ResponseEntity<>(new ApiResponse(false, errorMsgFromMatchingSignature), HttpStatus.BAD_REQUEST);
		}
